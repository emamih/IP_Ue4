...
public class ImageView<E> extends JScrollPane{
	...
	private double maxViewMagnification = 1.0;		// use 0.0 to disable limits ..needs from zoom
	private boolean keepAspectRatio = true;
	private boolean centered = true;
	private int zoom =2;
	int pixels[] = null;		// pixel array in ARGB format

	private boolean conturesVisible = false;
	private boolean polyVisible = false;
	private Vector<Path> conturePaths;
	private Vector<Path> polyPaths;

	public Vector<Path> getPolyPaths() {
		return polyPaths;
	}
	public void setPolyPaths(Vector<Path> polyPaths) {
		this.polyPaths = polyPaths;
	}
	public boolean isPolyVisible() {
		return polyVisible;
	}
	public void setPolyVisible(boolean selected) {
		this.polyVisible = selected;
	}


	public boolean isConturesVisible() {
		return conturesVisible;
	}
	/**
	 * @return the zoom
	 */
	...
	public void setConturesVisible(boolean conturesVisible) {
		this.conturesVisible = conturesVisible;
	}


	public Vector<Path> getConturePaths() {
		return conturePaths;
	}
	public void setConturePaths(Vector<Path> conturePaths) {
		this.conturePaths = conturePaths;
	}
	...
	class ImageScreen extends JComponent {
		...
		public void paintComponent(Graphics g) {
			...
			if (image != null) {
				...
				if(maxViewMagnification > 0.0) {
					int maxWidth = (int)(w * maxViewMagnification + 0.5);
					int maxHeight = (int)(h * maxViewMagnification + 0.5);
				...
				// keep aspect ratio
				if(keepAspectRatio) {
					double ratioX = (double)r.width / w;
					double ratioY = (double)r.height / h;
					if(ratioX < ratioY)
						r.height = (int)(ratioX * h + 0.5);
					else
						r.width = (int)(ratioY * w + 0.5);
				}
				...
				if (getZoom()>3) {
					...
					for (int yRasta = offsetY; yRasta < offsetY + r.height; yRasta+=getZoom()) {
						g.drawLine(offsetX ,yRasta, offsetX+r.width,yRasta);
					}
					for (int xRasta = offsetX; xRasta < offsetX + r.width; xRasta+=getZoom()) {
						g.drawLine(xRasta, offsetY+r.height,xRasta,offsetY);
					}
				...
				//draw contures
				if(conturesVisible){
					float angleFactor = 1.0f/conturePaths.size();
					int i =1;

					g2.setStroke(new BasicStroke(2));

					for (Path p : conturePaths){
						if(p.isInner()){
							g.setColor(new Color(Color.HSBtoRGB(angleFactor*i, 0.8f,0.8f)));
							i++;
						} else {
						...
						for(Edge edge : p){
							...
							Point end 	= new Point((edge.getEndPosition())%image.getWidth(), edge.getEndPosition()/image.getWidth());
							g2.drawLine(start.x*getZoom()+offsetX, start.y*getZoom()+offsetY, end.x*getZoom()+offsetX, end.y*getZoom()+offsetY);
				...
				if(polyVisible){
					...
					for (Path p : polyPaths){
						...
						for(Edge edge : p){
							Point start = new Point((edge.getStartPosition())%image.getWidth(), edge.getStartPosition()/image.getWidth());
							Point end 	= new Point((edge.getEndPosition())%image.getWidth(), edge.getEndPosition()/image.getWidth());
							g2.fillOval( start.x*getZoom()-5+offsetX, start.y*getZoom()-5+offsetY, 10, 10);
							g2.drawLine(	start.x*getZoom()+offsetX, start.y*getZoom()+offsetY, 
											end.x*getZoom() +offsetX , end.y*getZoom()+offsetY);
						}
					}
				}
			}
		...
		public Dimension getPreferredSize() {
			if(image != null) 
				return new Dimension(image.getWidth()*getZoom(), image.getHeight()*getZoom());
			else
